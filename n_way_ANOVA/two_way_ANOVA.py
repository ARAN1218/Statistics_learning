def two_way_ANOVA(df):
    ...
    # 分散分析表を作成する
    df_S = pd.Series([S1, S2, Se])
    df_df = pd.Series([df1, df2, dfe])
    ...
    df_ANOVA = pd.concat([df_S, df_df, df_V, df_F, df_p], axis=1).set_axis(['S','df','V','F','p','sign'], axis=1).set_axis(['Index', 'Columns','Error']).fillna('')
